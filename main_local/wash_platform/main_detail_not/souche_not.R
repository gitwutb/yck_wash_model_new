...
yck_czp<-dbFetch(dbSendQuery(loc_channel,"SELECT id car_id,brand brand_name,series series_name,model model_name,emission discharge_standard,displacement liter FROM spider_www_souche a
                             INNER JOIN (SELECT a.id_data_input FROM analysis_match_id a where a.car_platform='souche' AND a.match_des='not') m ON a.id=m.id_data_input;"),-1)
...
if(length(list_result)>1){
  ...
  if(nrow(match_right)>0){
    write.csv(return_db,paste0(deep_local,"\\file\\output\\souche.csv",sep=""),row.names = F,fileEncoding = "UTF-8",quote = F)
    ...
    loc_channel<-dbConnect(MySQL(),user = local_defin$user,host=local_defin$host,password= local_defin$password,dbname=local_defin$dbname)
    dbSendQuery(loc_channel,'SET NAMES gbk')
    dbSendQuery(loc_channel,paste0("LOAD DATA LOCAL INFILE '",deep_local,"/file/output/souche.csv'",
