...
if(as.integer(format(Sys.Date(),"%d"))>22){
  min_model_year=as.integer(format(Sys.Date(),"%Y"))-10}else{
    min_model_year=as.integer(format(Sys.Date(),"%Y"))-6
  }
...
if(weekdays(Sys.Date())=='星期四'){
  ...
  #车300及详细配置更新
  tryCatch({fun_vdata_sync(local_defin_yun,'config_che300_major_info',min_model_year)},error=function(e){fun_mailsend("车型库同步异常（云）-车300",e)},finally={0})
  tryCatch({fun_vdata_sync_detail_c300(min_model_year)},error=function(e){fun_mailsend("车型库同步异常（云）-车300detail",e)},finally={0})  
  ...
  tryCatch({fun_vdata_sync_detail_au(min_model_year)},error=function(e){fun_mailsend("车型库同步异常（云111）-汽车之家详细配置",e)},finally={0})
}
