...
fun_match_result<-function(che300,data_input){
  ...
  qx_data_input<-inner_join(qx_data_input,wutb_id,c("X"="wutb_id"))
  comp_result<-NULL
  x<-c(5,7,9,10,4,12,6,11,13:21,17)
  for (i in 18:4) {
...
fun_iteration<-function(wutb,confidence=NULL,qx_data_input,qx_che300,a){
  che_result<-fun_match(qx_data_input,qx_che300,a)
  wutb<-rbind(wutb,che_result)
  #去掉车300重复
  linshi_cid<-data.frame(car_id=setdiff(qx_che300$car_id,unique(wutb$id_che300)),stringsAsFactors = F)
  ...
  wutb$rname<-as.character(wutb$rname)
  ...
  qx_che300<-inner_join(qx_che300,linshi_cid,by="car_id")
  #去掉车data_input重复
  linshi_rid<-data.frame(X=setdiff(qx_data_input$X,unique(wutb$id_data_input)),stringsAsFactors = F)
  qx_data_input<-inner_join(qx_data_input,linshi_rid,by="X")
...
fun_match<-function(qx_data_input,qx_che300,a){
  comb_data_input<-qx_data_input[,3]
  comb_che300<-qx_che300[,3]
  for (j in a) {
    comb_data_input<-paste(comb_data_input,qx_data_input[,j],sep = "")
    comb_che300<-paste(comb_che300,qx_che300[,j],sep = "")
  }
  comb_data_input<-gsub(" ","",comb_data_input)
  comb_che300<-gsub(" ","",comb_che300)
  comb_data_input_match<-unique(data.frame(x=comb_data_input,id_data_input=qx_data_input$X,rname=qx_data_input$car_model_name))
  comb_che300_match<-unique(data.frame(x=comb_che300,id_che300=qx_che300$car_id,cname=qx_che300$car_model_name))
  comb_data_input_match$x<-as.character(comb_data_input_match$x)
  comb_che300_match$x<-as.character(comb_che300_match$x)
  #匹配
  return_match<-inner_join(comb_data_input_match,comb_che300_match,by="x")
  return(return_match)
}